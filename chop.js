...
// ==================== IN-MEMORY STORE ====================
var store = { _data: {} };
store.get = async function(k) { return this._data[k] || null; };
store.put = async function(k, v) { this._data[k] = v; };
...
async function initBQ() {
  try {
    const auth = new google.auth.GoogleAuth({
      keyFile: '/opt/nostream_bq/.gcp-sa.json',
      scopes: ['https://www.googleapis.com/auth/bigquery.insertdata']
    });
    google.options({ auth });
    bqReady = true;
    console.log('BQ connection ready');
  } catch(e) {
    console.log('BQ init skipped:', e.message);
  }
}

async function logToBQ(type, payload) {
  if (!bqReady) return;
  try {
    const rows = [{
      json: {
        process_type: 'chop_mvp',
        payload: JSON.stringify({ type, ...payload, ts: new Date().toISOString() }),
        timestamp: new Date().toISOString()
      }
    }];
    await bigquery.tabledata.insertAll({
      projectId: 'replit-gcp',
      datasetId: 'replit_scraper_raw',
      tableId: 'queue_master_payloads',
      requestBody: { rows }
    });
  } catch(e) {
    console.error('BQ log error:', e.message);
  }
}
...
async function homePage(req) {
  return new Response(shell(
    ...
    'var cp=null;var ct=localStorage.getItem("chop_ot")||"";var cpid=localStorage.getItem("chop_pid")||"";'+
    // On load, check if we have stored tokens to reconnect
    '(async function(){'+
    'if(ct&&cpid){'+
    'try{var r=await fetch("/api/reconnect?t="+ct);var d=await r.json();if(r.ok&&d.id){cp=d;showProject(d)}else{localStorage.removeItem("chop_ot");localStorage.removeItem("chop_pid")}}catch(e){console.log("reconnect fail")}'+
    '}'+
    // Also check URL params for deep-link
    'var up=new URLSearchParams(window.location.search);var upt=up.get("t");var upp=up.get("p");'+
    'if(upt&&upp&&!cp){'+
    'try{var r2=await fetch("/api/reconnect?t="+upt);var d2=await r2.json();if(r2.ok&&d2.id){localStorage.setItem("chop_ot",upt);localStorage.setItem("chop_pid",upp);cp=d2;showProject(d2)}}catch(e){}'+
    '}'+
    '})();'+
    ...
    'localStorage.setItem("chop_ot",d.owner_token);localStorage.setItem("chop_pid",d.id);cp=d;showProject(d);window.history.replaceState(null,"","/?p="+d.id+"&t="+d.owner_token)}catch(e){showToast("Error: "+e.message)}'+
    ...
    'var e=document.getElementById("expert-email").value.trim();var pid=localStorage.getItem("chop_pid");var tok=localStorage.getItem("chop_ot");'+
    ...
    'document.getElementById("expert-name").value="";document.getElementById("expert-email").value="";'+
    'var exp=d.expert;var sc="pending",st="Pending";'+
    'var el=document.getElementById("expert-list");var ll=document.getElementById("expert-links");'+
    'var emptyMsg=el.querySelector(\'[style*=\"color:#666\"]\');if(emptyMsg)emptyMsg.remove();'+
    'el.innerHTML+=\'<div class=\"expert-card\"><div><div class=\"name\">\'+exp.name+\'</div><div style=\"font-size:0.75rem;color:#666;margin-top:2px\">0/\'+(exp.total_questions||\'?\')+\' answered</div></div><div style=\"display:flex;align-items:center;gap:8px\"><span class=\"status \'+sc+\'\">\'+st+\'</span></div></div>\';'+
    'll.style.display="block";if(!ll.querySelector("label"))ll.innerHTML=\'<label>Share Links</label>\';'+
    'll.innerHTML+=\'<div style=\"display:flex;align-items:center;gap:8px;margin-bottom:6px;font-size:0.85rem\"><span style=\"min-width:120px\">\'+exp.name+\'</span><code class=\"inline-code\" style=\"flex:1\">\'+window.location.origin+\'/answer/\'+exp.token+\'</code></div>\';'+
    'var pa=document.getElementById("progress-area");var sb=document.getElementById("synthesize-btn");'+
    'if(pa){pa.style.display="block";var tot=el.querySelectorAll(".expert-card").length;var done=el.querySelectorAll(".status.done").length;pa.innerHTML=\'<div style=\"margin-top:8px;padding-top:12px;border-top:1px solid #333\"><div class=\"progress\"><span>People: \'+tot+\'</span><span>Done: \'+done+\'</span></div></div>\'}'+
    'showToast("Added "+exp.name)}'+
    'async function refreshExperts(){var pid=localStorage.getItem("chop_pid");var tok=localStorage.getItem("chop_ot");'+
    ...
    'async function triggerSynth(){var pid=localStorage.getItem("chop_pid");var tok=localStorage.getItem("chop_ot");var b=document.getElementById("synthesize-btn");b.disabled=true;b.textContent="Synthesizing...";'+
    'try{var r=await fetch("/api/projects/"+pid+"/synthesize",{method:"POST",headers:{"Content-Type":"application/json","X-Owner-Token":tok}});var d=await r.json();if(!r.ok)throw new Error(d.error||"Failed");'+
        ...
        'o.innerHTML=\'<div class="card"><h3>Context-Engineered Markdown</h3><pre id="md-out">\'+escHtml(d.markdown)+\'</pre><button class="btn btn-sm" onclick="copyMd()" style="margin-top:8px">Copy MD</button></div>\'+\'<div class="card"><h3>OKF Bundle</h3><pre id="okf-out">\'+escHtml(d.okf_bundle)+\'</pre><button class="btn btn-sm" onclick="copyOkf()" style="margin-top:8px">Copy OKF Bundle</button></div>\';showToast("Synthesis complete!")}catch(e){showToast("Error: "+e.message)}'+
        ...
        'function copyOkf(){var p=document.getElementById("okf-out");navigator.clipboard.writeText(p.textContent).then(function(){showToast("Copied!")})}'+
    'function escHtml(s){return s.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}'
  ), {headers:{'Content-Type':'text/html'}});
}

async function answerPage(req, token) {
...
async function createProject(req) {
  ...
  var questions = await generateQuestionsFromSeed(body.seed);
  ...
  logEvent('project_created', {id, name, qs:questions.length});
  return json({...project});
}

async function addExpert(req, pid) {
  ...
  logEvent('expert_added', {pid, name:body.name});
  return json({expert});
}

async function listExperts(req, pid) {
  var ot = req.headers.get('X-Owner-Token');
  var pStr = await store.get('project:'+pid);
  if (!pStr) return json({error:'Not found'}, 404);
  var p = JSON.parse(pStr);
...
async function getProject(req, pid, ot) {
  var pStr = await store.get('project:'+pid);
  if (!pStr) return json({error:'Not found'}, 404);
  var p = JSON.parse(pStr);
  if (ot && p.owner_token !== ot) return json({error:'Unauthorized'}, 403);
  var experts = JSON.parse(await store.get('project_experts:'+pid) || '[]');
  return json({...p, experts});
}

async function expertQuestions(req, token) {
  ...
  if (expert.status === 'pending' || expert.status === 'sent') {
    ...
    updateExpert(expert.project_id, expert.id, {status:'in_progress'});
    logEvent('expert_started', {pid:expert.project_id, name:expert.name});
  }
...
async function submitAnswer(req, token) {
  var body = await req.json();
  var eStr = await store.get('expert:'+token);
  if (!eStr) return json({error:'Not found'}, 404);
  var expert = JSON.parse(eStr);
  ...
  if (remaining === 0) {
    ...
    updateExpert(expert.project_id, expert.id, {status:'completed', answered});
    logEvent('expert_completed', {pid:expert.project_id, name:expert.name, answered, skipped});
  } else {
    updateExpert(expert.project_id, expert.id, {answered});
  }
  await store.put('expert:'+token, JSON.stringify(expert));
  logEvent(body.skip ? 'answer_skipped' : 'answer_submitted', {pid:expert.project_id, name:expert.name, q:as[idx].qid});
  var ci = as.findIndex(function(a){return !a.answered && !a.skipped});
  return json({assignments:as, current_index:ci >= 0 ? ci : as.length});
}

async function synthesize(req, pid) {
  var ot = req.headers.get('X-Owner-Token');
  var pStr = await store.get('project:'+pid);
  if (!pStr) return json({error:'Not found'}, 404);
  var p = JSON.parse(pStr);
  ...
  // Group by category for findings
  var byCat = {};
  for (var k2 = 0; k2 < groups.length; k2++) {
    var cat = groups[k2].qid.split('-')[1] || 'other';
    if (!byCat[cat]) byCat[cat] = [];
    byCat[cat].push(groups[k2]);
  }

  // Findings section
  md += '## Synthesis: Key Findings\n\n';
  for (var catName in byCat) {
    var catQs = byCat[catName];
    for (var ci = 0; ci < catQs.length; ci++) {
      var catG = catQs[ci];
      // Check for consensus
      var answers = catG.answers;
      if (answers.length > 0) {
        var findingText = answers[0].a.slice(0, 80) + (answers[0].a.length > 80 ? '...' : '');
        md += '### Finding (' + catG.qid + ')\n';
        md += findingText + '\n';
        if (answers.length > 1) {
          md += '**Consensus:** ' + answers.length + ' experts answered this question\n';
        } else {
          md += '**Source:** 1 expert\n';
        }
        md += '**Tags:** `#' + catName.toLowerCase() + '`\n\n';
      }
    }
  }
  ...
  md += 'When answering based on this document:\n';
  md += '1. Prefer Key Findings over individual answers\n';
  md += '2. Flag conflicts by citing both respondents\n';
  md += '3. Use tags (`#scope`, `#people`) to route to relevant sections\n';
  md += '4. When asked "who knows about X", cite the respondent by name\n\n';
  md += '--- End of Document ---\n';

  p.status = 'synthesized';
  await store.put('project:'+pid, JSON.stringify(p));
  logEvent('synthesis_completed', {pid, respondents:experts.filter(function(e){return e.answered>0}).length, answers:all.length});

  // Also build OKF bundle
  var bundle = '---\n';
  bundle += 'type: KnowledgeBundle\n';
  bundle += 'title: ' + p.name + '\n';
  bundle += 'tags: [chop, knowledge-capture]\n';
  bundle += 'source: Chop MVP\n';
  bundle += 'capture_date: ' + now + '\n';
  bundle += 'questions_asked: ' + (p.questions.length || all.length) + '\n';
  bundle += 'questions_answered: ' + all.length + '\n';
  bundle += 'respondents: ' + experts.filter(function(e){return e.answered > 0}).length + '\n';
  bundle += '---\n\n';
  bundle += md;

  return json({markdown: md, okf_bundle: bundle});
}
...
// Helpers
async function updateExpert(pid, eid, updates) {
...
async function logEvent(type, payload) {
  try {
    ...
    // Also log to BQ
    logToBQ(type, payload).catch(function(){});
  } catch(e) { console.error('log:', e); }
}

// ==================== AI QUESTION GENERATION ====================
async function generateQuestionsFromSeed(seed) {
  var apiKey = process.env.OPENROUTER_API_KEY || '';
  if (!apiKey) {
    console.log('No OPENROUTER_API_KEY, using DEFAULT_QUESTIONS');
    return DEFAULT_QUESTIONS;
  }
  var payload = JSON.stringify({
    model: 'openai/gpt-4o-mini',
    messages: [
      {role: 'system', content: "You generate knowledge capture questionnaires. Given a seed topic, generate 8-12 questions across these categories: SCOPE (what's in/out), PERSONA (who needs this), PROCESS (how it works), GAP (what's missing), PEOPLE (who knows), FAILURE (what goes wrong), SOURCE (where's truth). Return ONLY a JSON array of objects with {qid, category, text} fields. qid format: Q-CATEGORY-NN (e.g. Q-SCOPE-01). Make questions answerable in 30-60 seconds."},
    ...
    var req = https.request(options, function(res) {
      ...
      res.on('end', function() {
        try {
          ...
          if (!content) {
            console.log('AI response empty, using DEFAULT_QUESTIONS');
            return resolve(DEFAULT_QUESTIONS);
          }
          // Try to extract JSON array from the response
          var match = content.match(/\[[\s\S]*\]/);
          var jsonStr = match ? match[0] : content;
          var questions = JSON.parse(jsonStr);
          if (!Array.isArray(questions) || questions.length < 1) throw new Error('Not an array');
          // Validate and normalize each question
          questions = questions.map(function(q, i) {
            return {
              qid: q.qid || ('Q-GEN-' + String(i+1).padStart(2,'0')),
              category: (q.category || 'general').toLowerCase(),
              text: q.text || 'Untitled question'
            };
          });
          console.log('Generated ' + questions.length + ' questions from AI');
          resolve(questions);
        } catch(e) {
          console.log('Failed to parse AI response, using DEFAULT_QUESTIONS:', e.message);
          resolve(DEFAULT_QUESTIONS);
        }
      });
    });
    req.on('error', function(e) {
      console.log('AI request failed, using DEFAULT_QUESTIONS:', e.message);
      resolve(DEFAULT_QUESTIONS);
    });
    req.write(payload);
    req.end();
...
async function handleRequest(req, res) {
  var url = new URL(req.url, 'http://localhost:8787');
  var path = url.pathname;
  var method = req.method;

  // Build Web API Request from Node req
  ...
  try {
    var response;
    if (method === 'OPTIONS') {
      response = new Response(null, {status:204, headers:{'Access-Control-Allow-Origin':'*','Access-Control-Allow-Methods':'GET,POST,PUT','Access-Control-Allow-Headers':'Content-Type,X-Owner-Token'}});
    }
    else if (path === '/api/projects' && method === 'POST') response = await createProject(request);
    else if (path === '/api/reconnect' && method === 'GET') {
      var ot = url.searchParams.get('t');
      if (!ot) { response = json({error:'Missing token'}, 400); }
      else {
        var pid = await store.get('owner:'+ot+':project');
        if (!pid) { response = json({error:'No project found for this token'}, 404); }
        else { response = await getProject(request, pid, ot); }
      }
    }
    else if (path === '/api/admin/events') response = await getEvents();
    else if ((path.match(/^\/api\/projects\/([^/]+)$/) || [])[0] && method === 'GET') {
      var pOt = url.searchParams.get('t') || null;
      response = await getProject(request, path.split('/')[3], pOt);
    }
    else if ((path.match(/^\/api\/projects\/([^/]+)\/experts$/) || [])[0] && method === 'POST') response = await addExpert(request, path.split('/')[3]);
    else if ((path.match(/^\/api\/projects\/([^/]+)\/experts$/) || [])[0] && method === 'GET') response = await listExperts(request, path.split('/')[3]);
    else if ((path.match(/^\/api\/projects\/([^/]+)\/synthesize$/) || [])[0] && method === 'POST') response = await synthesize(request, path.split('/')[3]);
    else if ((path.match(/^\/api\/answer\/([^/]+)\/questions$/) || [])[0] && method === 'GET') response = await expertQuestions(request, path.split('/')[3]);
    else if ((path.match(/^\/api\/answer\/([^/]+)\/submit$/) || [])[0] && method === 'POST') response = await submitAnswer(request, path.split('/')[3]);
    else if (path === '/admin') response = await adminPage(request);
    else if ((path.match(/^\/answer\/([^/]+)$/) || [])[0]) response = await answerPage(request, path.split('/')[2]);
    else response = await homePage(request);
...
server.listen(PORT, '0.0.0.0', async function() {
  console.log('🍪 Chop MVP server running');
